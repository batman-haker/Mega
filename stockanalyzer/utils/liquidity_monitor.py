...
class LiquidityMonitor:
    ...
    def __init__(self, fred_api_key: Optional[str] = None):
        ...
        self.series = {
            ...
            'bank_assets': 'TLAACBW027SBOG',  # Total Assets of Commercial Banks (Kostecki!)
            'tga': 'WTREGEN',            # Treasury General Account
            'tga_alt': 'WDTGAL',         # Treasury General Account (alternatywa)
        ...
        self.thresholds = {
            # Reserves/Assets Ratio
            'reserves_assets_abundant': 13.0,    # >13% = ABUNDANT (nadmierna płynność)
            'reserves_assets_ample_high': 13.0,  # 12-13% = AMPLE (cel Fed)
            'reserves_assets_ample_low': 12.0,
            'reserves_assets_below_ample': 11.0, # 11-12% = PONIŻEJ AMPLE (napięcia)
            'reserves_assets_scarce': 10.0,      # <10% = SCARCE (kryzys!)

            # Rezerwy (absolutne - stare progi)
            'reserves_minimum': 2800,  # mld USD - minimum "wystarczajace"
            'reserves_comfortable': 3000,  # mld USD - komfortowy poziom

            # SOFR-IORB Spread
            'sofr_iorb_spread_stable': 0.05,     # <5 bps = bardzo stabilne
            'sofr_iorb_spread_normal': 0.10,     # 5-10 bps = normalne warunki
            'sofr_iorb_spread_warning': 0.15,    # 10-15 bps = OSTRZEŻENIE (napięcia!)
            'sofr_iorb_spread_stress': 0.20,     # 15-20 bps = STRESS (poważne napięcia)
            'sofr_iorb_spread_critical': 0.20,   # >20 bps = KRYZYS (Fed musi interweniować!)
        ...
        # === SYSTEM WAG ===
        # Wagi według Dan Kostecki (2024-2025) - framework oparty na NY Fed Research
        # UWAGA: Kostecki używa PROGÓW ABSOLUTNYCH, nie percentyli!
        self.indicator_weights = {
            # === TIER 1 - CRITICAL (65%) - Bezpośrednie wskaźniki płynności ===
            'reserves_assets_ratio': 0.35,  # 35% - #1! Reserves/Assets (NY Fed Staff Report 1019)
            'sofr_iorb_spread': 0.30,       # 30% - #2! Spread = napięcia repo (real-time stress)

            # === TIER 2 - IMPORTANT (20%) - Mechaniczny wpływ na płynność ===
            'tga_change': 0.20,             # 20% - #3! TGA delta = transfer 1:1 do rezerw

            # === TIER 3 - CONTEXTUAL (10%) - Warunki makro ===
            'fed_balance': 0.05,            # 5% - QT/QE/RMP tracking
            'reverse_repo': 0.05,           # 5% - Bufor płynności (obecnie wyczerpany)

            # === TIER 4 - SUPPORTING (5%) - Dodatkowy kontekst ===
            'yield_curve': 0.02,            # 2% - Sygnał recesji
            'vix': 0.02,                    # 2% - Sentyment
            'nfci': 0.01,                   # 1% - Warunki finansowe
        }

        # STARE WAGI (zachowane dla kompatybilności z istniejącym kodem)
        self.indicator_weights_legacy = {
            'sofr_iorb_spread': 0.20, 'reserves': 0.15, 'yield_curve': 0.15,
            'vix': 0.10, 'nfci': 0.10, 'hy_spread': 0.10,
            'tga': 0.08, 'reverse_repo': 0.07, 'fed_balance': 0.05,
            'm2': 0.04, 'dollar_index': 0.03, 'treasury_10y': 0.03,
            'inflation_5y': 0.03, 'unemployment': 0.02,
        }
    ...
    def calculate_percentiles(self, indicators: Dict) -> Dict:
        ...
        print(f"\n[PERCENTILES] Obliczono percentyle dla {len(percentiles)} wskaznikow")
    ...
    def detect_correlations_and_conflicts(self, indicators: Dict) -> Dict:
        ...
        if len(drain_signals) >= 2:
            patterns['compound_signals'].append({
                ...
                'name': '[WARN] LIQUIDITY DRAIN (Wielokrotny odpływ)',
        ...
        if len(credit_signals) >= 2:
            patterns['compound_signals'].append({
                'type': 'credit_crunch',
                'name': '[ALERT] CREDIT CRUNCH WARNING',
            ...
            if not data.empty:
                try:
                    ...
                    if len(data) > 1:
                        ...
                        # Bezpieczne filtrowanie - konwersja do timestamp jeśli potrzeba
                        latest_date = pd.to_datetime(latest['date']) if not isinstance(latest['date'], pd.Timestamp) else latest['date']
                        week_ago = data[pd.to_datetime(data['date']) <= latest_date - timedelta(days=7)]
    ...
    def calculate_reserves_assets_ratio_DISABLED(self, indicators: Dict) -> Optional[float]:
        """
        Oblicza Reserves/Assets Ratio - #1 wskaźnik Dan Kostecki

        Formula: (WRESBAL / TLAACBW027SBOG) * 100

        Returns:
            Ratio w % lub None jeśli brak danych
        """
        if 'reserves_alt' not in indicators or 'bank_assets' not in indicators:
            return None

        reserves = indicators['reserves_alt']['current']
        assets = indicators['bank_assets']['current']

        if assets == 0:
            return None

        ratio = (reserves / assets) * 100
        return ratio

    def calculate_tga_change_30d_DISABLED(self, indicators: Dict) -> Optional[float]:
        """
        Oblicza zmianę TGA w ciągu 30 dni

        TGA↑ = Reserves↓ (negatywne)
        TGA↓ = Reserves↑ (pozytywne)

        Returns:
            Delta w mld USD lub None jeśli brak danych
        """
        if 'tga' in indicators:
            # Preferujemy change_30d jeśli istnieje
            if 'change_30d' in indicators['tga']:
                return indicators['tga']['change_30d']
            # Fallback na change_7d * 4 (przybliżenie)
            elif 'change_7d' in indicators['tga']:
                return indicators['tga']['change_7d'] * 4
    ...
    def analyze_liquidity_conditions(self, indicators: Dict) -> Dict:
        ...
        print(f"[REGIME] {regime['name']} (mnoznik: {regime['multiplier']}x)")
        ...
        weighted_score = 0  # Kostecki methodology weighted scoring
        ...
        # 1A. KOSTECKI #1 - Reserves/Assets Ratio (35% weight)
        if 'reserves_alt' in indicators and 'bank_assets' in indicators:
            reserves_val = indicators['reserves_alt']['current']
            assets_val = indicators['bank_assets']['current']

            if assets_val > 0:
                ratio = (reserves_val / assets_val) * 100
                ratio_score = 0

                # Thresholds per Dan Kostecki + NY Fed
                if ratio >= self.thresholds['reserves_assets_abundant']:  # >13%
                    ratio_score = +100  # ABUNDANT - excessive liquidity
                    analysis['signals'].append({
                        'type': 'positive',
                        'indicator': 'Reserves/Assets',
                        'message': f'ABUNDANT: {ratio:.1f}% - Excessive liquidity, cheap leverage',
                    })
                elif ratio >= self.thresholds['reserves_assets_ample_low']:  # 12-13%
                    ratio_score = +50  # AMPLE - Fed target zone
                    analysis['signals'].append({
                        'type': 'positive',
                        'indicator': 'Reserves/Assets',
                        'message': f'AMPLE: {ratio:.1f}% - Fed target zone, stable conditions',
                    })
                elif ratio >= self.thresholds['reserves_assets_below_ample']:  # 11-12%
                    ratio_score = 0  # BELOW AMPLE - early tensions
                    analysis['signals'].append({
                        'type': 'neutral',
                        'indicator': 'Reserves/Assets',
                        'message': f'BELOW AMPLE: {ratio:.1f}% - Early tensions appearing',
                    })
                elif ratio >= self.thresholds['reserves_assets_scarce']:  # 10-11%
                    ratio_score = -50  # SCARCE - serious stress
                    analysis['alerts'].append({
                        'severity': 'warning',
                        'indicator': 'Reserves/Assets',
                        'message': f'SCARCE: {ratio:.1f}% - Serious liquidity stress!',
                    })
                else:  # <10%
                    ratio_score = -100  # CRISIS
                    analysis['alerts'].append({
                        'severity': 'critical',
                        'indicator': 'Reserves/Assets',
                        'message': f'CRISIS: {ratio:.1f}% - Fed must intervene immediately!',
                    })

                # Apply 35% weight
                weighted_contribution = ratio_score * self.indicator_weights['reserves_assets_ratio']
                weighted_score += weighted_contribution

                # Store for debugging
                if 'weighted_scores' not in analysis:
                    analysis['weighted_scores'] = {}
                analysis['weighted_scores']['reserves_assets_ratio'] = {
                    'raw_score': ratio_score,
                    'weight': self.indicator_weights['reserves_assets_ratio'],
                    'contribution': weighted_contribution,
                    'ratio_percent': ratio,
                }

        # 2A. KOSTECKI #3 - TGA Change 30-day (20% weight) - Momentum indicator
        if 'tga' in indicators:
            tga = indicators['tga']['current']
            # Prefer 30-day change, fallback to 7-day * 4
            tga_change_30d = indicators['tga'].get('change_30d') or indicators['tga'].get('change_7d', 0) * 4
            tga_score = 0

            # TGA momentum interpretation per Kostecki:
            # Rising TGA = drains liquidity (bearish)
            # Falling TGA = adds liquidity (bullish)

            if tga_change_30d > 150:  # Large drain >150B/month
                tga_score = -100  # Very bearish
                analysis['signals'].append({
                    'type': 'negative',
                    'indicator': 'TGA Momentum',
                    'message': f'TGA DRAINING: +${tga_change_30d:.0f}B/month - Major liquidity removal!',
                })
            elif tga_change_30d > 75:  # Moderate drain 75-150B
                tga_score = -50
                analysis['signals'].append({
                    'type': 'negative',
                    'indicator': 'TGA Momentum',
                    'message': f'TGA rising: +${tga_change_30d:.0f}B/month - Draining liquidity',
                })
            elif tga_change_30d > -75:  # Stable -75 to +75B
                tga_score = 0  # Neutral
                analysis['signals'].append({
                    'type': 'neutral',
                    'indicator': 'TGA Momentum',
                    'message': f'TGA stable: ${tga_change_30d:+.0f}B/month - Neutral impact',
                })
            elif tga_change_30d > -150:  # Moderate injection -150 to -75B
                tga_score = +50
                analysis['signals'].append({
                    'type': 'positive',
                    'indicator': 'TGA Momentum',
                    'message': f'TGA falling: {tga_change_30d:.0f}B/month - Adding liquidity',
                })
            else:  # Large injection <-150B
                tga_score = +100  # Very bullish
                analysis['signals'].append({
                    'type': 'positive',
                    'indicator': 'TGA Momentum',
                    'message': f'TGA INJECTING: {tga_change_30d:.0f}B/month - Major liquidity add!',
                })

            # Apply 20% weight
            weighted_contribution = tga_score * self.indicator_weights['tga_change']
            weighted_score += weighted_contribution

            if 'weighted_scores' not in analysis:
                analysis['weighted_scores'] = {}
            analysis['weighted_scores']['tga_change'] = {
                'raw_score': tga_score,
                'weight': self.indicator_weights['tga_change'],
                'contribution': weighted_contribution,
                'change_30d': tga_change_30d,
            }

        # 2. KOSTECKI #2 - SOFR-IORB Spread (30% weight) - Real-time stress indicator
        if 'sofr_iorb_spread' in indicators:
            spread = indicators['sofr_iorb_spread']['current']
            spread_bps = spread * 100
            spread_change = indicators['sofr_iorb_spread'].get('change_7d', 0)
            spread_score = 0

            # Thresholds per Dan Kostecki
            if spread > self.thresholds['sofr_iorb_spread_critical']:  # >20 bps
                spread_score = -100  # CRISIS - Fed must intervene
                analysis['alerts'].append({
                    'severity': 'critical',
                    'indicator': 'SOFR-IORB Spread',
                    'message': f'REPO CRISIS: {spread_bps:.1f} bps - Fed must intervene! Basis trade unwinding risk!',
                })
            elif spread > self.thresholds['sofr_iorb_spread_stress']:  # 15-20 bps
                spread_score = -75  # STRESS - serious tensions
                analysis['alerts'].append({
                    'severity': 'critical',
                    'indicator': 'SOFR-IORB Spread',
                    'message': f'REPO STRESS: {spread_bps:.1f} bps - Serious tensions! Expensive leverage!',
                })
            elif spread > self.thresholds['sofr_iorb_spread_warning']:  # 10-15 bps
                spread_score = -50  # WARNING - tensions starting
                analysis['alerts'].append({
                    'severity': 'warning',
                    'indicator': 'SOFR-IORB Spread',
                    'message': f'TENSIONS: {spread_bps:.1f} bps - Rising repo costs, basis trade under pressure',
                })
            elif spread > self.thresholds['sofr_iorb_spread_normal']:  # 5-10 bps
                spread_score = +25  # Normal conditions
                analysis['signals'].append({
                    'type': 'positive',
                    'indicator': 'SOFR-IORB Spread',
                    'message': f'NORMAL: {spread_bps:.1f} bps - Standard conditions',
                })
            else:  # <5 bps
                spread_score = +100  # VERY STABLE - abundant liquidity
                analysis['signals'].append({
                    'type': 'positive',
                    'indicator': 'SOFR-IORB Spread',
                    'message': f'VERY STABLE: {spread_bps:.1f} bps - Excessive liquidity, cheap leverage',
                })

            # Apply 30% weight
            weighted_contribution = spread_score * self.indicator_weights['sofr_iorb_spread']
            weighted_score += weighted_contribution

            if 'weighted_scores' not in analysis:
                analysis['weighted_scores'] = {}
            analysis['weighted_scores']['sofr_iorb_spread'] = {
                'raw_score': spread_score,
                'weight': self.indicator_weights['sofr_iorb_spread'],
                'contribution': weighted_contribution,
                'value': spread,
                'value_bps': spread_bps,
            }

            # Additional alert if spread rising quickly
            if spread_change > 0.05:  # increase > 5 bps per week
                analysis['alerts'].append({
                    'severity': 'warning',
                    'indicator': 'SOFR-IORB Trend',
                    'message': f'Spread rising fast: +{spread_change*100:.1f} bps/week - Tensions escalating!',
                })
        ...
        # === FINALNE SCORING ===
        # Add Kostecki weighted scores (35% R/A + 30% SOFR + 20% TGA)
        ...
        print(f"[KOSTECKI] Weighted score: {weighted_score:+.1f} (35% R/A + 30% SOFR + 20% TGA)")
        print(f"[CORRELATIONS IMPACT] Score adjustment: {correlation_adjustment:+.0f} (Total przed rezimem: {score:.0f})")
        ...
        analysis['kostecki_weighted_score'] = weighted_score  # Kostecki methodology
